        ...
        try:
            ...
            if self.resize[0] == -1:
                if self.word:
                    ...
                    image = image.resize(
                        (int(image_width * (height / image_height)), height)
                    )
            else:
...
class GetLoader:
        ...
        if os.path.exists(self.project_path):
            ...
            if os.path.exists(self.cache_path):
                ...
                if not os.path.exists(self.cache_train_path):
                    logger.error(
                        "\nCache Train File {} is not exists!".format(
                            self.cache_train_path
                        )
                    )
                    exit()
    ...
    def collate_to_sparse(self, batch):
        ...
        images_pad = []
        for img in images:
            img = torchvision.transforms.Pad((0, 0, int(max_width - img.size[0]), 0))(
                img
            )
